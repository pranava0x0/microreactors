...
- **A partition/skip-list only protects one wave — a later, independent pass has no visibility into an earlier pass's records unless it's explicitly pointed at them.** (2026-08-29) A gap-fill agent run days after the original research pass re-researched a deal (Intel Ohio/AEP Ohio) the original pass had already written up, producing two records with the same real deal but conflicting numbers — one conflated a $95.1M substation's actual 50 MW capacity with the buyer's eventual 500 MW full-site draw, both real figures from the same source, attributed to the wrong thing. A second, same-wave duplicate (two agents independently found the identical grant) sat invisible for even longer, since neither the citation gate nor the quote-verification gate catches it: those only prove a quoted span exists on the page, not that two records don't describe the same thing, or that a paraphrased field (`capacity`, `price`) attributes the right number to the right quantity. Both duplicates surfaced only when an unrelated downstream feature (a UI cross-link) aggregated records along a new dimension. **Run an explicit post-merge duplicate sweep** (group by shared source URL, then by name/region/date) as a standing step before shipping any merged research dataset — don't rely on partitioning discipline to have prevented it.
...
1. **Find the real endpoint.** Fetch the service's own help/index and read the URL template off it. Don't trust a documented or guessed path.
   *No help/index to read (undocumented SPA API)?* Three moves, cheapest first (2026-08-23, NRC ADAMS — the old documented adams.nrc.gov endpoint no longer resolves; the working one is `POST adams-search.nrc.gov/api/search`): (1) grep the SPA's `main.*.js` bundle for `"/api/` literals; (2) POST `{}` to the candidate and read the validation errors — a well-built API names its own required fields ("The 'filters' property is required…"); (3) if the body is assembled dynamically, drive the UI once in the browser pane and read the real request off `read_network_requests`. Minutes, in that order; guessing body shapes first wastes the afternoon.
...
- **Some hosts 403 non-browser TLS fingerprints regardless of UA string** (2026-08-23: nrc.gov/docs, *.af.mil, oklo.com, ktoo.org — while the same agencies' API subdomains fetch fine). The in-app browser pane is the fetcher of last resort: `get_page_text` for HTML, then index the capture with provenance marked out-of-band (microreactors: `fetch_source.py --from-file`). For PDFs, navigate the tab to the PDF URL so the origin matches, then same-origin `fetch()` via `javascript_tool` into a `window` global.
- **Oversized `javascript_tool` returns are a feature: a base64 blob past the token cap is auto-saved to a tool-results file, so chunked PDF bytes never enter context.** Slice the Uint8Array (~60KB/chunk — deliberately over the cap), then decode from the saved files. The saved `text` field is not bare base64 — it is `"<b64>"\n\n(captured at origin https://…)`, so split on the quotes before decoding, reassemble in chunk order, and verify total byte count + `%PDF-` magic before trusting the file.
