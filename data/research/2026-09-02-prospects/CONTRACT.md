...
## Type C — `answer` (added 2026-09-02: a finding on a named open question)
The prospects on the site's Deals › Prospects sub-tab (`data/strategy.json`) each end with a
`next_question`. An answer record closes, narrows or confirms the absence of one of them.
```json
{"_meta": {"captured": "2026-09-02", "agent": "<your slug>", "scope": "<one line>",
           "angles_run": ["..."], "absences": ["what you looked for and could NOT verify"]},
 "answers": [{
   "id": "kebab-slug",
   "for": "<prospect id from data/strategy.json, e.g. doyon-utilities>",
   "question": "the open question, copied verbatim from the seed digest",
   "status": "answered" | "partial" | "absent",
   "finding": "2-5 sentences. What the fetched document actually says, with the number, the date and who said it. For partial: what is known and what is not. For absent: say so in one sentence.",
   "figures": {"any_named_number": "verbatim value with unit, e.g. '$0.6498/kWh (FY2024, fuel only)'"},
   "searched": ["angle 1", "angle 2"],
   "sources": [ ...universal source objects; at least one status: fetched unless status is absent... ]
 }]}
```
- `for` must be a prospect id that exists; the validator rejects any other value.
- An `answered` or `partial` record needs at least one FETCHED source. A snippet is a lead, never a finding.
- An `absent` record needs `searched` (the angles you actually ran) and no sources are required.
- A number that appears in `finding` must also sit in `figures`, verbatim, so the integrator can lift it without re-reading.
- Also write Type B `case` records when a finding is itself a priced deal, rate order or award at a named site (a tariff order with $/kWh, a contract with a value): those become rows on Costs › Customer cost. Use `"sector": "Remote outposts & microgrids"` for Alaska and Nunavut records and `"Off-grid mining & mineral processing"` for mines.
